...
- **Story knowledge (#50):** Occurrence-first JSONL corpus per `memory_scope_id` with rebuildable TF-IDF semantic index; `story_occurrence` / `story_derived` information classes; evidence-budget `full_eligible` / `semantic_ranked`; live epistemic eligibility via `PublicEvent.known_by` at query; Librarian `mediation_outcome` distinguishes failure modes from `no_match`. Projected occurrence rows may carry `evidence_projection` manifest (#51). **Maintainer doc:** [docs/story-knowledge.md](./docs/story-knowledge.md). **Forensic record:** [governance/records/issue-50-story-knowledge-forensic-record.md](./governance/records/issue-50-story-knowledge-forensic-record.md).
- **Committed occurrence evidence (#51):** Promoted `PublicEvent` rows are a durable committed-occurrence/evidence boundary (not a duplicate Continuity store). Optional `occurrence_evidence` preserves bounded multi-producer contributions, user-trigger provenance, fact refs, and scoped private evidence. Classification describes occurrence **kind**; semantic evidence preserves **what happened**. `known_by` remains live retrieval authority.
- **Narrator environmental B2 (#49):** Authored environmental canon (compiled retrieval / setup snapshot) is **immutable** during story execution. Continuity-bearing perceptible properties may become **story-derived environmental descriptors** via Host-accepted B2 establishment → `#50` derived records. Supersession selects current effective value while preserving history. **Truth establishment** (`orchestration_only` establishment decision) is distinct from **Character epistemic access** (`known_by` / Librarian eligibility).
