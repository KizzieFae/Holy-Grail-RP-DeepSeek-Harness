...
**Opener** — Authored **opening prose** asset, referenced from bootstrap `opening` when strategy is asset-based; not a substitute for **Template** structure. In **Streamlit**, the sidebar lists openers for template- / character-asset modes; **label** and **description** (file-level) are shown, and an explicit **selection** is required when more than one opener exists in the same scope (GitHub **#101**). Headless runs use manifest/composition inputs instead of that UI.
