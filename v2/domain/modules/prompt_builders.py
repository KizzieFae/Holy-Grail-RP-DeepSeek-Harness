...
- Each ``type: action`` beat has non-empty ``action`` (visible self-only, third person). Ordinary observable physical action may omit ``recipients`` (defaults to ``present`` scope for everyone currently present). Concealed, restricted, or named-recipient physical action must include explicit ``recipients`` with the correct scope: ``present`` for ordinary observable action, ``directed`` with non-empty ``characters`` for named-recipient action, ``private`` with non-empty ``characters`` for actor-only or explicitly entitled concealed action. Hidden physical action remains ``type: action`` (not motivation or internal thought). Each ``type: speech`` beat has non-empty ``dialogue``. On speech beats, optional ``audibility`` is one of ``public``, ``directed``, ``private``; omit for public. For ``directed`` or ``private``, include non-empty ``audience`` (names). For public, omit ``audience`` or use ``[]``.
