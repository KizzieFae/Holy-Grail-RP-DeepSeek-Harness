...
import { runTranche2Campaign } from '../src/scenario-harness/tier1-tranche2.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const reportPath = process.argv[2]
  ?? path.join(__dirname, '..', 'tmp', 'tranche2-report.json');

async function main() {
  if (!hasLiveApiKey()) {
    console.error('DEEPSEEK_API_KEY not set; cannot execute Tranche 2 live campaign.');
    process.exit(2);
  }
  const report = await runTranche2Campaign();
