/**
 * Issue #201 Package D Stage 3 — preamble decomposition (D-01a + D-01b only).
 * Reuses D0 controls and Stage-2 D-01 (EXP-1) evidence for blind packet assembly.
 */
...
const STAGE2_EVIDENCE_ROOT = path.join(
  REPO_ROOT,
  'data',
  'investigation_runs',
  'issue201-package-d-stage2-2026-09-14T08-09-42-791Z',
);
...
const EXPERIMENTS = [
  {
    experiment_id: 'D-01b',
    maps_to: 'D-01b',
    label: 'Storyteller-only bypass (Plot retained)',
    roundOptions: { skipStorytellerCognition: true },
    info_preservation: {
      cognition_removed: [
        'storyteller_orientation',
        'storyteller_assessment',
        'librarian_mediation (storyteller lane)',
      ],
      preserved: [
        'plot_cognition_init',
      ...
      disappears: ['storyteller advisory package', 'storyteller-mediated librarian bundle'],
      confound: 'low-moderate — plot cognition and authoritative manifest remain',
    },
  },
  {
    experiment_id: 'D-01a',
    maps_to: 'D-01a',
    label: 'Plot-only bypass (Storyteller retained)',
    roundOptions: { skipPlotCognitionOrchestration: true },
    info_preservation: {
      cognition_removed: ['plot_cognition_init', 'plot_cognition_update', 'plot resume lifecycle'],
      preserved: [
        'storyteller_orientation',
        'storyteller_assessment',
        'storyteller-mediated librarian',
        'scenario premise',
        'character cards',
        'continuity',
        'PVR',
      ...
      disappears: ['plot cognition resume summaries', 'plot-mediated advisory updates'],
      confound: 'low-moderate — storyteller advisory remains; plot state machinery may still exist authoritatively',
    },
  },
];
...
function kindCountFromKinds(kinds, name) {
  return (kinds.find((k) => k.inference_kind === name)?.count ?? 0);
}

function summarizeStorytellerPlotKinds(kinds) {
  const list = kinds ?? [];
  ...
  return {
    ...
    storyteller_kinds: storytellerKinds,
    plot_kinds: plotKinds,
  };
}
...
function loadD0Controls() {
  ...
  return report.runs.map((run) => {
    ...
    const sp = summarizeStorytellerPlotKinds(kinds);
    const inference = {
      ...run.inference,
      librarian_mediation_count: kindCountFromKinds(kinds, 'librarian_mediation'),
      character_orientation_count: kindCountFromKinds(kinds, 'character_orientation'),
      director_semantic_qa_count: kindCountFromKinds(kinds, 'director_semantic_qa'),
      storyteller_inference_count: sp.storyteller_inference_count,
      plot_inference_count: sp.plot_inference_count,
    };
    return {
      case_id: run.case_id,
      ...
      reused_from: D0_EVIDENCE_ROOT,
    };
  });
}

function loadReusedD01Combined() {
  const stage2Outputs = path.join(STAGE2_EVIDENCE_ROOT, 'outputs');
  const runs = [];
  for (const scenarioKey of ['arkham_stress', 'ayame_controlled']) {
    for (let rep = 1; rep <= 2; rep += 1) {
      const caseId = `EXP-1-${scenarioKey}-r${rep}`;
      const metaPath = path.join(stage2Outputs, `${caseId}-meta.json`);
      const presentationPath = path.join(stage2Outputs, `${caseId}-presentation.txt`);
      if (!fs.existsSync(metaPath) && fs.existsSync(presentationPath)) {
        runs.push({
          case_id: caseId,
          experiment_id: 'D-01',
          scenario_key: scenarioKey,
          scenario_id: SCENARIOS[scenarioKey].id,
          repetition_index: rep,
          presentation_path: presentationPath,
          presentation_text: fs.readFileSync(presentationPath, 'utf8'),
          reused_from: STAGE2_EVIDENCE_ROOT,
          backfilled: true,
        });
        continue;
      }
      if (!fs.existsSync(metaPath)) throw new Error(`Missing reused D-01 evidence: ${metaPath}`);
      const meta = JSON.parse(fs.readFileSync(metaPath, 'utf8'));
      runs.push({
        ...meta,
        experiment_id: 'D-01',
        presentation_text: meta.presentation_text ?? fs.readFileSync(meta.presentation_path, 'utf8'),
        reused_from: STAGE2_EVIDENCE_ROOT,
      });
    }
  }
...
function summarizeAttempts(attempts) {
  ...
  const kinds = llmKinds.map(([kind, rows]) => ({
    inference_kind: kind,
    count: rows.length,
...
async function observeWorldState({ api, hgSceneId, history, userEntry, presentation, scenarioId }) {
  ...
  observations.push({ type: 'history_entry_count', count: history?.entries?.length ?? 0 });
  return observations;
}
...
function compareToD0(controlRuns, variantRun) {
  ...
  const ctrlSt = controls.map((c) => c.inference.storyteller_inference_count ?? 0);
  const ctrlPl = controls.map((c) => c.inference.plot_inference_count ?? 0);
  return {
    control_case_ids: controls.map((c) => c.case_id),
    delta_inference_count: variantRun.inference.inference_count - avg(ctrlInf),
    delta_wall_ms: variantRun.operation_wall_ms - avg(ctrlWall),
    delta_librarian_mediation: variantRun.inference.librarian_mediation_count - avg(ctrlMed),
    delta_storyteller_inferences: variantRun.inference.storyteller_inference_count - avg(ctrlSt),
    delta_plot_inferences: variantRun.inference.plot_inference_count - avg(ctrlPl),
  };
}

function buildHumanBlindPacket(allCases, outputsDir) {
  const samples = allCases.map((c) => ({
    blind_label: null,
    case_id: c.case_id,
  ...
  samples.forEach((s, i) => { s.blind_label = String.fromCharCode(65 + i); });
  const packetPath = path.join(outputsDir, 'issue201-stage3-human-blind-eval-packet.json');
  const keyPath = path.join(outputsDir, 'issue201-stage3-human-blind-eval-answer-key.json');
  const packet = {
    schema: 'issue201_blind_eval_packet_v1',
    purpose: 'Package D Stage 3 — preamble decomposition blind eval (D0 + D-01 + D-01a + D-01b)',
    instructions: 'Score presentation_text only. Same 11-dimension rubric as Stage 2. Do not request architecture, experiment, latency, or cost data until after scoring.',
  ...
  return { packetPath, keyPath, sample_count: samples.length };
}
...
async function main() {
  ...
  const evidenceRoot = cli.evidenceRoot
    ?? path.join(REPO_ROOT, 'data', 'investigation_runs', `issue201-package-d-stage3-${stamp}`);
  const outputsDir = path.join(evidenceRoot, 'outputs');
  fs.mkdirSync(outputsDir, { recursive: true });

  const d0Controls = loadD0Controls();
  const reusedD01 = loadReusedD01Combined();

  const report = {
    schema: 'issue201_package_d_stage3_preamble_decomposition_v1',
    ...
    d0_controls_reused: true,
    d01_combined_reused: true,
    d0_controls: d0Controls.map((c) => ({
      ...
      storyteller_inference_count: c.inference.storyteller_inference_count,
      plot_inference_count: c.inference.plot_inference_count,
      librarian_mediation_count: c.inference.librarian_mediation_count,
      reused_from: c.reused_from,
    })),
    d01_combined_reused_runs: reusedD01.map((r) => ({
      case_id: r.case_id,
      scenario_key: r.scenario_key,
      operation_wall_ms: r.operation_wall_ms ?? null,
      inference_count: r.inference?.inference_count ?? null,
      ...
      for (let rep = 1; rep <= 2; rep += 1) {
        ...
        if (cli.skipExisting && fs.existsSync(presentationPath) && fs.statSync(presentationPath).size > 0) {
          ...
          if (fs.existsSync(metaPath)) {
            ...
            stub.comparison_to_d0 = compareToD0(d0Controls, stub);
            expReport.runs.push(stub);
          }
          continue;
        }
        if (fs.existsSync(presentationPath) && fs.statSync(presentationPath).size === 0) {
        ...
        try {
          ...
          result.comparison_to_d0 = compareToD0(d0Controls, result);
          expReport.runs.push(result);
          console.log(JSON.stringify({
            caseId,
            wall_ms: result.operation_wall_ms,
            inferences: result.inference.inference_count,
            storyteller: result.inference.storyteller_inference_count,
            plot: result.inference.plot_inference_count,
          ...
          expReport.runs.push(failure);
          console.error(JSON.stringify({ caseId, failed: true, error: failure.error }));
        }
      }
    }
    if (expReport.runs.length > 0) report.experiments.push(expReport);
  }
  ...
  const matrixCases = [
    ...
    ...reusedD01,
    ...report.experiments.flatMap((e) => e.runs.filter((r) => r.presentation_path || r.presentation_text)),
  ];
  report.human_blind_eval = buildHumanBlindPacket(matrixCases, outputsDir);
  report.human_blind_eval.status = 'prepared_for_governance_blind_scoring';
  report.preamble_matrix_runtime = {
    D0_control: report.d0_controls,
    D01_combined_reused: report.d01_combined_reused_runs,
    D01a: report.experiments.find((e) => e.experiment_id === 'D-01a')?.runs ?? [],
    D01b: report.experiments.find((e) => e.experiment_id === 'D-01b')?.runs ?? [],
  };

  const outPath = path.join(evidenceRoot, 'issue201-package-d-stage3-preamble-decomposition-report.json');
