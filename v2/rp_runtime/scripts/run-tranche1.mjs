...
import { runTranche1Campaign } from '../src/scenario-harness/tier1-tranche1.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const reportPath = process.argv[2]
  ?? path.join(__dirname, '..', 'tmp', 'tranche1-report.json');

async function main() {
  if (!hasLiveApiKey()) {
    console.error('DEEPSEEK_API_KEY not set; cannot execute Tranche 1 live campaign.');
    process.exit(2);
  }
  const report = await runTranche1Campaign();
