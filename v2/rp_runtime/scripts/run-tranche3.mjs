...
import {
  runTranche3Campaign,
  TRANCHE3_INFERENCE_CEILING,
} from '../src/scenario-harness/tier1-tranche2.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const reportPath = process.argv[2]
  ?? path.join(__dirname, '..', 'tmp', 'tranche3-report.json');

async function main() {
  if (!hasLiveApiKey()) {
    console.error('DEEPSEEK_API_KEY not set; cannot execute Tranche 3 live campaign.');
    process.exit(2);
  }
  const report = await runTranche3Campaign();
  ...
  console.log(JSON.stringify({
    ...
    call_ceiling: TRANCHE3_INFERENCE_CEILING,
