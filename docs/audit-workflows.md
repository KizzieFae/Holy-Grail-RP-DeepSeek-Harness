...
**Round latency reconstruction (#158, #173):** `python tools/investigation/reconstruct_round_latency.py <hg_session_id> [--operation <operation_id>] [--round <hg_round_id>] [--commit <domain_commit_id>] [--attribution]` rebuilds Player-operation elapsed time (when application lifecycle + real `operation_id` exist), authoritative inference timings (`dsh_session_turn_boundary`), orchestration span/join evidence (`decision.orchestration_graph`), and derived critical-path attribution with explicit `attribution_scope` (`post_commit_section`, `character_turn`, `round_internal`, `player_visible_operation`) and confidence (`proven` | `bounded_partial` | `unavailable`). `--commit` scopes character-turn reconstruction; omit it for round-internal scope. Use DSH turn event times and span `execution.*` fields for ordering — not evidence write order (`recorded_at`). Do not equate round-internal critical path with Player-visible latency unless both scopes are measured. Pre-#173 sessions and other incomplete graphs remain `bounded_partial` on per-attempt timing only. Unattributed remainder is a valid analytical output.
