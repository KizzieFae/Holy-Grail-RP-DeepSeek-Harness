...
- **Narrator manifest** (`kernel.prepare_narrator_context`): committed-move rendering surface plus bounded scene setup/state/progression, **`narrator_environment_baseline`** (`EnvironmentalCurrentView` — authored + story-derived B2, deterministic, zero LLM), **`triggering_user_context`** (#51 occurrence evidence), and optional **`narrator_environment_cognition`** audit from pre-render cognition (#49). Pre-render: DSH **`runNarratorEnvironmentCognition`** → structured N1/N2 → targeted Librarian (`consumer_role=narrator`) when baseline insufficient → B2 establishment via `submit_derived_record` → render. B1 ephemeral texture is not persisted. Does not receive Director scratch or Character-private lanes.
