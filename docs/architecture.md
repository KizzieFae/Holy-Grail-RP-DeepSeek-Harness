...
- **Player decomposition (#91, #109):** DSH `runPlayerDecompositionPhase` calls Host `prepare_player_decomposition_context` for a minimal `PromptContributionManifest` whose `inference_instruction` imports `PLAYER_DECOMPOSITION_OUTPUT_INSTRUCTION` from `narrative_visibility_prompt.py`. The ephemeral player-authored turn remains in the DSH user/task prompt (`PLAYER SOURCE`); deterministic validation remains in `player_perceptual_service.py` at `record_user_turn`.
