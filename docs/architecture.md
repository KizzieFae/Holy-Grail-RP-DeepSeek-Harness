...
**Live post-commit S4 (#39, authority contract #100; semantic producer realignment #164):** on every successful Character commit, DSH runs Narrator presentation and the producer-neutral post-commit semantic path (**`runLibrarianProposalGeneration`**) in parallel from the same commit, then **joins** the semantic branch (finalize + persist + terminal audit) before the next **`getEligibleActors`** / Director cycle—including multi-commit rounds (per-commit join, not round-end only). **#164:** routine synchronous production is **Storyteller** narrow assessment (`storyteller_post_commit_issue_pressure`, 0–1 LLM per commit); **eligibility gate** skips inference when no ACTIVE/ESCALATING issues exist. Host finalize paths persist session state and **`librarian_proposal_audit_log`** under per-session locks; this is a **second persistence seam** after normal `commit_move`, not a second unrestricted turn-commit authority.
...
|---------|---------------|
| `PublicEvent.revelation_significance_by_character` | `knowledge_revelation_significance` (legacy/separate invocation; not routine sync #164) |
| `ContinuityManager.issue_pressure_semantic_overlays` | `issue_tension_pressure` (routine sync #164) |
...
S4 **must not** modify `turn_counter`, authoritative `IssueState`, or `known_by`. **#164:** `consequence_meaning` and `information_salience` are **retired** as post-commit LLM kinds (S2a salience unchanged); routine sync does not emit `knowledge_revelation_significance`. At-most-once: terminal audit per `domain_commit_id` via `find_terminal_audit_for_commit`. See [PACKET_CONTRACTS.md](../PACKET_CONTRACTS.md).
...
**Character perceptual inventory (#199):** Character context includes an authoritative **`authoritative_perceptual_inventory`** manifest lane assembled by `character_perceptual_inventory.py`. It lists the **authorized sensory substrate** available to the acting Character at the current scene point — entitled perceptual units, applicable visible grounding facts, and established observable `player_fact:*` sources. The inventory may be **explicitly empty** (master text states no authorized Player sensory evidence is available). Private/scenario knowledge lanes (`character_private`, authored character knowledge) may support inference but **do not** create sensory evidence. Semantic evaluation enforces the boundary via **R02b** (unsupported Player sensory/physiological/emotional-display claims) and **R14** (entitlement). Fallible interpretation anchored to entitled substrate remains allowed. Normative contract: [player-authorship-authority.md](./player-authorship-authority.md).

**Scene-pressure freshness and bidirectional Player-action authority (#200):** Librarian `issue_tension_pressure` semantic overlays stored in `ContinuityManager.issue_pressure_semantic_overlays` are **derived/advisory** dramatic pressure — not authoritative world/action truth. Objective overlay fields (`semantic_unmet_condition`, `stakes_summary`) bind `player_authority_sequence_at_apply` at apply time and are **structurally freshness-bound** to tier-1 Player authority: when a newer committed Player post has a higher `rp_history` user `sequence_index`, stale objective fields are withheld from projection until the overlay is refreshed/revalidated (no lexical reconciliation). Character `scene_pressures` packaging includes a compact precedence note so derived pressure does not override newer authoritative Player facts. **R16** protects against both unsupported forward Player-action completion (#193) and **objective regression** of established Player action/state; Character ignorance, deception, and distinct subsequent requirements remain legal. Character epistemics remain separately governed by perception/entitlement (#155, #199). **Generalized Player contribution → Continuity/world-state promotion does not exist** — tier-1 Player posts establish action truth for semantic authority and freshness sequencing, not automatic durable world-state mutation. Implementation: `continuity_scene_pressure_projection.py`, `continuity_librarian_issue_pressure.py`, `player_action_completion_authority.py`. Forensic join: [audit-workflows.md](./audit-workflows.md).
