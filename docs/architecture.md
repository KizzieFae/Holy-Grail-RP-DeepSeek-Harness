...
- **Storyteller (#32)** — bounded advisory narrative cognition: orientation → Librarian bundle → informed assessment → `StorytellerAdvisoryPackage` → (S3b) deterministic Packaging mapper → suggestive `storyteller_*` lanes. **S3a implemented:** isolated cognition loop + Host validation + audit chain. **S3b implemented:** `storyteller_packaging_mapper.py` with validity gate and per-consumer policies. **Not implemented:** live `prepare_*` / round-orchestrator wiring (S3c). Does **not** control plot outcomes, `next_actor`, Character intent, Narrator events, retrieval, information mediation, or persistence. `PreservationSignal` is an attention hint only and is not mapped to consumer lanes. Invalidated packages are **not** injected into new `prepare_*` calls.
...
Character suggestive retrieval only: `KnowledgeService.project_context` + `retrieval_selection.py` caps → manifest contributions. Director/Narrator receive authoritative continuity projections and digests; no Librarian bundles or Storyteller lanes in live role manifests. Storyteller S3a/S3b produce advisory packages and mapped contributions through isolated Host test surfaces only. See [PACKET_CONTRACTS.md](../PACKET_CONTRACTS.md) **RetrievedContextBundle** for the live retrieval seam.
