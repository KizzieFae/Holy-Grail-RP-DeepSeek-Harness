...
- **Player decomposition (#91, #109, #120):** DSH `runPlayerDecompositionPhase` calls Host `prepare_player_decomposition_context` for a minimal `PromptContributionManifest` whose `inference_instruction` imports `PLAYER_DECOMPOSITION_OUTPUT_INSTRUCTION` from `narrative_visibility_prompt.py`. The ephemeral player-authored turn remains in the DSH user/task prompt (`PLAYER SOURCE`); deterministic validation remains in `player_perceptual_service.py` at `record_user_turn`. **Player `internal` (#120):** intrinsically nonperceptual player information (cognition, private state, nonperceptual explanatory narration, and other non-observable narrative context) — never `public` scope; hard-excluded from Character projection. Concealed physical actions remain `observable_event` + restrictive scope.
