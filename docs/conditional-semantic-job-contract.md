...
- **Job vs attempt:** semantic jobs are opened from trigger/eligibility; inference success is not required to establish or attach attempts. Failed inference attempts remain on the same `semantic_job_id`; token/time accounting stays on each attempt record.
- **Session index:** `semantic_jobs.by_semantic_job_id[<id>].canonical_evidence_id` is the authoritative handle for canonical envelope recovery (including QA infrastructure retries).
