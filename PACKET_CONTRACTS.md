...
- **`StorytellerAdvisoryPackage`** — round-scoped advisory narrative assessment (**S3a cognition implemented**: orientation, Librarian consumption, informed assessment, Host validation, round-stable validity metadata). **S3b Packaging mapper implemented:** deterministic `storyteller_*` suggestive lanes for Director/Character/Narrator consumer policies via `storyteller_packaging_mapper.py` (Narrator policy is a validated future socket). **S3c live integration (Model A):** round-start cognition + round-local bind; live injection while valid serves **Director + Character**; commit invalidates before Narrator.

### Current implementation seam

**RetrievedContextBundle** (below) remains the **live Character retrieval-to-packaging path**. **Librarian S2b** adds the mapper seam but does **not** replace Character retrieval/memory lanes or wire into `kernel.prepare_context` yet. **Storyteller S3a/S3b/S3c (Model A):** round-start package maps to suggestive `storyteller_*` lanes for **Director and scoped Character** while valid; authoritative commit invalidates the package before Narrator, so Narrator normally receives no Storyteller advisory. Optional future post-commit refresh could activate the validated S3b Narrator mapper. Storyteller does not affect actor selection or Continuity truth.
