...
**#32 S3c (Storyteller live round integration):** DSH **`runStorytellerCognition`** runs once after **`hg/round-started`** and before the first Director attempt; Host **`bind_storyteller_advisory_package`** stores round-local advisory state; **`kernel.prepare_director_context`**, **`prepare_context`**, and **`prepare_narrator_context`** inject validated S3b mapper output while the package remains valid; authoritative **`commit_move`** invalidates the package (`authoritative_commit`) so later role contexts receive no Storyteller lanes. Storyteller remains optional — failures skip advisory injection and baseline runtime continues. **Not implemented:** #34 write-side proposals, unbounded refresh loops, actor-selection authority changes.
...
- **`StorytellerAdvisoryPackage`** — round-scoped advisory narrative assessment (**S3a cognition implemented**: orientation, Librarian consumption, informed assessment, Host validation, round-stable validity metadata). **S3b Packaging mapper implemented:** deterministic `storyteller_*` suggestive lanes for Director/Character/Narrator via `storyteller_packaging_mapper.py`. **S3c live integration implemented:** round-start cognition hook + round-local bind + `prepare_*` injection while valid; invalidated after authoritative commit.

### Current implementation seam

**RetrievedContextBundle** (below) remains the **live Character retrieval-to-packaging path**. **Librarian S2b** adds the mapper seam but does **not** replace Character retrieval/memory lanes or wire into `kernel.prepare_context` yet. **Storyteller S3a/S3b/S3c** produce advisory packages, map them to suggestive `storyteller_*` lanes, and inject them into live Director/Character/Narrator manifests while the round-local package remains valid. Storyteller does not affect actor selection or Continuity truth.
