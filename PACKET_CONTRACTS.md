...
**#39 live post-commit orchestration (implemented):** after each successful Character commit, DSH **`hg-round-orchestrator`** runs **Narrator** and post-commit semantic lifecycle **concurrently**. A **per-commit join** (`hg/post-commit-semantic-join`) completes semantic disposition (including Host **`SessionRepository.persist`**) before the next Director/eligibility cycle. Host uses per-**`hg_scene_id`** session locks; **`librarian_proposal_audit_log`** (legacy container name) persists canonical **`post_commit_semantic_*`** audit entries for replay/at-most-once protection. Test-only **`skipLibrarianProposalGeneration`** may suppress the path.

**#164 forensic contracts (implemented):** `hg_post_commit_semantic_v1` (`decision.post_commit_semantic` on inference attempts); `hg_post_commit_semantic_disposition_v1` (deterministic non-inference skip evidence); `hg_runtime_build_provenance_v1` (descriptive build/source metadata); `hg_runtime_effective_configuration_v1` (resolved configuration epochs + SHA-256 fingerprint). LLM attempts and disposition records carry `correlation.effective_configuration_epoch_id` referencing the governing `epochs[].epoch_id` when captured. Per-attempt `request.inference_profile` remains authoritative for actual model/provider; session provenance complements without overriding.
