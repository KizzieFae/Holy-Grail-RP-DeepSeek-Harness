...
- **`StorytellerAdvisoryPackage`** — round-scoped advisory narrative assessment (**S3a cognition implemented** at `f688448+`: orientation, Librarian consumption, informed assessment, Host validation, round-stable validity metadata). **S3b Packaging mapper implemented:** deterministic `storyteller_*` suggestive lanes for Director/Character/Narrator via `storyteller_packaging_mapper.py`. **Live consumer wiring not implemented:** round-orchestrator / `prepare_*` injection remains S3c.

### Current implementation seam

**RetrievedContextBundle** (below) remains the **live Character retrieval-to-packaging path**. **Librarian S2b** adds the mapper seam but does **not** replace Character retrieval/memory lanes or wire into `kernel.prepare_context` yet. **Storyteller S3a** produces validated advisory packages; **S3b** maps them to suggestive `storyteller_*` lanes in isolation. Neither injects into Director/Character/Narrator live manifests or actor selection until **S3c**.
