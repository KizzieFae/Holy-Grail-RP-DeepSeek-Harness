...
- **`StorytellerAdvisoryPackage`** — round-scoped advisory narrative assessment (**S3a cognition implemented** at `850b137+`: orientation, Librarian consumption, informed assessment, Host validation, round-stable validity metadata). **Consumer wiring not implemented:** deterministic Packaging mapping to per-role `storyteller_*` suggestive lanes and round-orchestrator injection remain future work.

### Current implementation seam

**RetrievedContextBundle** (below) remains the **live Character retrieval-to-packaging path**. **Librarian S2b** adds the mapper seam but does **not** replace Character retrieval/memory lanes or wire into `kernel.prepare_context` yet. **Storyteller S3a** produces validated advisory packages through isolated Host/DSH surfaces; it does **not** inject into Director/Character/Narrator manifests or actor selection.
