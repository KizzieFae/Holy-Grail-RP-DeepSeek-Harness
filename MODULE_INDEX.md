...
**#77 / #81 posture:** **#77** Slice **A** (validated) = foundational continuity seam + **`ContinuityPromptProjectionV77`** + API excursion scaffolding. **#81** (closed) = **`continuity_mutation_pipeline`** + canonical **`SceneState.location`** (**Slice A**), excursion lifecycle on **`process_turn`** (**Slice B**), and **close + reintegration** merges with atomic/idempotent semantics (**Slice C**). Further atoms / broader **#33/#34** product paths = follow-on issues if needed.

| Module | Responsibility | Interacts with | Notes |
|--------|----------------|----------------|-------|
| `continuity_mutation_pipeline.py` | **#81** typed mutation requests, D→S→M composer per `CanonicalAtom`, spatial **M** extraction from `spatial_transition`; excursion lifecycle + reintegration payload application | `continuity_state` (`SceneState`), `continuity_reintegration` | Called from `continuity_manager.process_turn`; `validate_spatial_transition_move_shape` in `response_validation_content` |
| `continuity_reintegration.py` | **#81 Slice C:** apply structured reintegration on excursion close; rollback on failure; commit id idempotency | `continuity_state`, `continuity_mutation_pipeline` | Invoked from close path in `continuity_manager` / pipeline |
| `continuity_manager.py` | Promote moves to events; issues; scene; interpretations; knowledge | `continuity_*_helpers`, `continuity_state`, `continuity_mutation_pipeline`, `perception_audibility`, `tension_pacing_policy` | Authoritative narrative state; **#81:** `process_turn` runs mutation composer (LOCATION, EXCURSION_LIFECYCLE, reintegration on close) before `_classify_turn_consequences`; `PublicEvent` knowability via `known_by`/`observed_by`; hybrid tension pacing + saturation gate in `_update_scene_state` |
