...
| Player-authorship authority / invented Player facts / Narrator Player-body attribution (#157) | [docs/player-authorship-authority.md](./docs/player-authorship-authority.md); Host `player_authorship_authority.py`, `semantic_evaluation_context.py`, `narrator_semantic_qa_context.py`; DSH `character-semantic-evaluation.mjs`, `narrator-semantic-qa.mjs`, `narrator-phase.mjs` |
| Unsupported Player sensory/physiological/emotional-display claims / `authoritative_perceptual_inventory` / Character perceptual grounding (#199) | [docs/player-authorship-authority.md](./docs/player-authorship-authority.md); Host `character_perceptual_inventory.py`, `character_upstream_context.py`, `semantic_evaluation_context.py`; domain `viewer_player_perception.py`; forensic recipe [audit-workflows.md](./docs/audit-workflows.md) |
| Player action completion / forward unsupported completion / objective regression of Player action (#193, #200 inverse R16) | [docs/player-authorship-authority.md](./docs/player-authorship-authority.md); Host `player_action_completion_authority.py`, `semantic_evaluation_context.py`; DSH `character-semantic-evaluation.mjs` |
| Wrong prompt / missing context | Host `continuity_context_projector.py`, role context modules (`director_context.py`, `character_context.py`, `narrator_context.py`, `opening_context.py`, `narrator_environment_context.py`, `context_substrate.py`, `storyteller_round_packaging.py`), `kernel.py` (`prepare_*` façades); Character Librarian path: `character_upstream_context.py`, `character_service.py`, `character-cognition-substrate.mjs`; legacy Character/Director prompt text in `prompt_builders.py`; Narrator render-instruction formatting in `narrator_render_instruction.py`; DSH `src/lib/live-inference-prompts.mjs` (Director/Character transport envelope only); DSH `src/plugins/hg-context-bridge/` (manifest transport) |
...
| Player turn perceptual decomposition / source accounting / failure handling (#91, #125 entitlement context, G-125-01 `role_private` projection) | DSH `player-decomposition-phase.mjs`, `perceptual-visibility-parse.mjs`; Host `player_decomposition_context.py` (`prepare_player_decomposition_context`), `player_pvr_entitlement_context.py` (`PlayerPvrEntitlementContextV1`), `player_entitlement_authority.py` (`entitlement_authority_snapshot`), `player_source_accounting.py`, `player_perceptual_service.py`, `player_perceptual_projection.py`, `perception_audibility_visibility.py` (`resolve_entitled_characters`), `kernel.record_user_turn`; API `POST /v1/sessions/player-decomposition/context/prepare`, `POST /v1/sessions/history/user-turn` (`player_decomposition`); execution evidence: manifest `contributions` including `player_pvr_entitlement_context` |
| Character committed-turn perceptual derivation / action recipients / PVR pre-commit (#92) | `character_move_ingress.py` (action `recipients`), `character_perceptual_service.py`, `character_perceptual_projection.py`; Host `kernel.validate_move`, `commit_move_transaction.py`, `session_history.py`, `memory_layer/writes.py`, `semantic_evaluation_context.py` |
| Narrator/opening Character-facing perception / perceptual visibility / degraded recovery / projection audit | `perceptual_visibility_contract.py`, `perceptual_visibility_validation.py`, `perceptual_visibility_projection.py`, `perceptual_visibility_legacy.py`; Host `perceptual_visibility_service.py`, `session_history.py` (`project_history_to_character_context_chat`); DSH `perceptual-visibility-parse.mjs`, narrator/opening/player-decomposition phase executors; API `/v1/perceptual-visibility/validate`, `/v1/sessions/opening/perceptual-visibility` |
...
| `scene_pressures` lacks Librarian semantic overlay / stale objective overlay fields after newer Player post (#200 freshness) | `continuity_scene_pressure_projection.py` (`overlay_semantic_fields_are_fresh`, `overlay_for_semantic_projection`), `continuity_librarian_issue_pressure.py` (`player_authority_sequence_at_apply`), `kernel.record_user_turn` (`note_authoritative_player_contribution`), `director_context_digests.py`, `character_context_projector.py`, `storyteller_orientation_context.py`; forensic recipe [audit-workflows.md](./docs/audit-workflows.md) |
...
| Plot Cognition overlay / init / update / replan / Character projection (#58–#64) | DSH `plot-cognition-orchestration.mjs`, `plot-cognition-update-substrate.mjs`, `plot-cognition-character-projection.mjs`, `character-epistemic-projection-eval.mjs`; Host `plot_cognition_orchestration_service.py`, `plot_cognition_projection_service.py`, `plot_cognition_update_service.py`, `plot_cognition_initialization_service.py`, `plot_cognition_overlay_service.py`, `plot_cognition_forensics_service.py`; contracts [docs/plot-cognition-orchestration-contract.md](./docs/plot-cognition-orchestration-contract.md); forensics `tools/investigation/trace_plot_cognition_forensics.py` |
| Inference transport / provider dispatch / execution evidence recording | DSH `src/lib/domain-api-client.mjs`, `src/plugins/hg-phase-executors/inference-substrate.mjs`, `src/plugins/hg-context-bridge/`, `src/lib/contract-correction-substrate.mjs`, `src/lib/execution-evidence/`; overview [docs/architecture.md](./docs/architecture.md) → Inference transport; catalog `src/application/llm-call-catalog.mjs` |
...
| Forensic execution evidence / actor selection / semantic QA chain | `tools/investigation/trace_turn_forensics.py` (unified navigator, #101); `tools/investigation/list_execution_evidence.py`; `v2/rp_runtime/src/lib/execution-evidence/`; conditional semantic jobs `v2/rp_runtime/src/lib/conditional-job/` (#215); [docs/conditional-semantic-job-contract.md](./docs/conditional-semantic-job-contract.md); [docs/rp-data-layout.md](./docs/rp-data-layout.md) (#28) |
...
| Prompt wording only (after ruling out state) | Character/Director canonical schema/domain: Host `director_context.py` / `character_context.py` (`inference_instruction`); DSH transport: `live-inference-prompts.mjs`; legacy reference: `prompt_builders.py`; Narrator live manifest assembly: `narrator_context.py`; Narrator render-instruction formatting: `narrator_render_instruction.py` |
