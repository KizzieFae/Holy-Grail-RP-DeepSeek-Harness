...
| `turn_runner_character_attempt.py` | Single-turn **attempt** slice: retry garnish, agent call, parse, normalize, validate (**#231** proposal coherence → **`validate_bot_response`** → presence override → **#232** proposal legality), structured retries, **`process_turn`** + progression gate, character audit payloads / `log_character_turn_audit` | Injected `validate_bot_response_fn`, `assess_proposal_beat_contradiction_fn`, `progression_enforcement`, continuity, `perception_audibility` (normalize only), `response_validation_*` helpers | v2 move object identity preserved where applicable; reject/forfeit → **no** narrator |
...
| `continuity_scene_state_update.py` | **`run_update_scene_state`** orchestration; **#232** centralized reconstruction suppression + accept-path proposal presence compile | `continuity_semantic_proposals`, `continuity_presence_pipeline`, `continuity_presence_helpers` | `_SUPPRESS_RECONSTRUCTION_COVERED_COMMITS` — deletion-friendly gate until **#235–#238** |
...
| `scene_exit_detection.py` | Hard departure **heuristic** signals (legacy reconstruction lane) | text / moves | **Not** covered-semantic commit authority post-#232; physical removal **#235** |
