...
| `audit_instrumentation.py` | **`RP_AUDIT_INSTRUMENTATION`** toggle; **`audit_instrumentation_enabled`**, **`log_audit_warning`**, **`log_audit_exception`** | callers in audit summary prep / logger | Writes to **`logging`** logger **`rp_app.audit`** only when instrumentation is enabled — configure handlers if operators need **`scan_audit_artifact_gaps`** warnings on console (**GitHub [#192](https://github.com/KizzieFae/Holy_Grail_RP/issues/192)** P2 instrumentation visibility prelude) |
