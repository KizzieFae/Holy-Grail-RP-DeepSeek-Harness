...
| **Authored retrieval** / `RetrievedContextBundle` / non-authoritative prompt block | `retrieved_context_select.py` (load + select + log), `app_turn_prompting.py` (single call site), `runtime_packets.py` (`RetrievedItem`, bundle, `format_retrieved_context_for_prompt`), `prompt_builders.py` (`retrieved_context_section`); env `RP_RETRIEVED_CONTEXT_INDEX` |
...
| `app_turn_prompting.py` | Character / Director / Narrator prompt assembly glue | `prompt_builders`, state, `progression_advisory`, `perception_audibility`, `offstage_prompt_filter`, `memory_layer.retrieval`, `prompt_derivations`, `retrieved_context_select`, `runtime_packets` (shadow) | Character `state_context` via `build_character_state_context_for_prompt` (identity + episodic); per-recipient transcript + structured moves; offstage narrowing; optional progression suffix; **Phase 2:** **only** call site for authored retrieval selection (`select_retrieved_context_bundle`); **Phase 0.5:** optional packet shadow compare (`RP_PACKET_SHADOW_COMPARE`) |
...
| `runtime_packets.py` | Read-only packet types (`RuntimeScenePacket`, `RuntimeCharacterPacket`, `RetrievedContextBundle` + `RetrievedItem`), `format_retrieved_context_for_prompt`, scene-state split/merge, live vs packet-derived **structured** prompt-input bundle compare | `app_turn_prompting` (shadow hook; passes `retrieved=` into packet build) | **Phase 2:** bundle carries authored retrieval; **no** selection logic here. **Shadow:** when `RP_PACKET_SHADOW_COMPARE` is `1`/`true`/`yes`; continuity + `CharacterState` remain authoritative |
| `retrieved_context_select.py` | **Phase 2–3.1** deterministic **authored JSON index** load + `select_retrieved_context_bundle`; `lore` lane; per-kind subcaps; lore truncation at selection; caps/dedup; `log_retrieval_if_active`; env path helper | `app_turn_prompting` (sole runtime caller), `runtime_packets` (types import) | **Not** vector/graph/transcript/dynamic memory; non-authoritative reference only |
| `authored_index_compile.py` | **Phase 3.1** offline `compile_authored_index`: manifest → `schema_version` 2 JSON (lossy; no `compiled_at`; OOC blocklist) | `scripts/compile_authored_retrieval_index.py`, tests | Manifest-driven only; no globs |
