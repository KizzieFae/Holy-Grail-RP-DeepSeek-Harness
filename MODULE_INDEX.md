...
| Player turn perceptual decomposition / source accounting / failure handling (#91, #125 entitlement context) | DSH `player-decomposition-phase.mjs`, `perceptual-visibility-parse.mjs`; Host `player_decomposition_context.py` (`prepare_player_decomposition_context`), `player_pvr_entitlement_context.py` (`PlayerPvrEntitlementContextV1`), `player_source_accounting.py`, `player_perceptual_service.py`, `player_perceptual_projection.py`, `kernel.record_user_turn`; API `POST /v1/sessions/player-decomposition/context/prepare`, `POST /v1/sessions/history/user-turn` (`player_decomposition`); execution evidence: manifest `contributions` including `player_pvr_entitlement_context` |
