...
| `app_turn_prompting.py` | Character / Director / Narrator prompt assembly glue | `prompt_builders`, state, `progression_advisory`, `perception_audibility`, `offstage_prompt_filter`, `memory_layer.retrieval`, `prompt_derivations`, `retrieved_context_select`, `episodic_memory_*` (cache/inputs/select when **`RP_EPISODIC_MEMORY`**), `runtime_packets` (shadow) | Character `state_context` via `build_character_state_context_for_prompt` (identity + memory-layer episodic sections); per-recipient transcript + structured moves; offstage narrowing; optional progression suffix; **Phase 2–3.2:** **only** call site for retrieval — authored (`select_retrieved_context_bundle`) or **merged** authored + bounded episodic (`merge_retrieved_context_with_episodic`); **Phase 0.5:** optional packet shadow compare (`RP_PACKET_SHADOW_COMPARE`) |
...
| `runtime_packets.py` | Read-only packet types (`RuntimeScenePacket`, `RuntimeCharacterPacket`, `RetrievedContextBundle` + `RetrievedItem`), `format_retrieved_context_for_prompt`, scene-state split/merge, live vs packet-derived **structured** prompt-input bundle compare | `app_turn_prompting` (shadow hook; passes `retrieved=` into packet build) | **Phase 2–3.2:** bundle may carry **authored + episodic** items in one lane (`source_kind` e.g. `episodic:public_event`); **no** selection logic here. **Shadow:** when `RP_PACKET_SHADOW_COMPARE` is `1`/`true`/`yes`; continuity + `CharacterState` remain authoritative |
