...
| Wrong prompt / missing context | Host `continuity_context_projector.py`, `kernel.py` (`prepare_context`, `prepare_director_context`, `prepare_narrator_context`); Character Librarian path: `character_upstream_context.py`, `character_service.py`, `character-cognition-substrate.mjs`; domain `prompt_builders.py`; DSH `src/plugins/hg-context-bridge/` (transport only) |
