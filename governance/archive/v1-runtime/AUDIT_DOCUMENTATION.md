...
**Character prompt proposal-schema teaching (#249, canonical default `v1_next7`):** Production character system prompts teach **`semantic_evaluation.proposals`** shape via `prompt_topology_issue240.py` (marker `proposal_schema_teaching_v249_a`). This aligns model output with **`issue240_semantic_evaluation.validate_issue240_semantic_evaluation_ingress`** — observational distinction: **schema reject** (forbidden keys / missing `character`) vs **semantic reject** vs **authority reject** remains in audit/validation metadata, not in #243 eval. Ontology/retry/missed-emission investigations: **#251** (landed canonical investigation topology: `simplified_structural_v1` on `v1_next7_issue251_physical_severance_guarded_v1` — see `validation_runs/issue251/CANONICAL_DOCTRINE.md`; **not** production default; historical hybrid `v1_next7_issue251_awareness_clean`; adjudicated replay via `i251_replay_adjudication.py`), **#250**. Evidence: `validation_runs/issue249/`, `validation_runs/issue251/`.
...
**Shipment vs specification (repo truth — GitHub #211):**

| Category | What it means today |
|----------|---------------------|
| **Shipped** | **`run_scene_eval_v1`** (`scene_eval_v1.py`, Issue **#66**) — character `*_full.json` rows only; optional **`structured_eval`** mirror judgments. Output includes **`scene_eval_version`** string **`"2"`**, which denotes the **#66 bundle / predicate-package revision** (GitHub **#141** move-shape predicate), **not** this Issue **#69** layer. |
| **Specified, deferred** | **`scene_eval_v2.py`**, **`run_scene_eval_v2`**, and **`<session_dir>/_scene_eval_v2.json`** — **not present** in the repository; no shipped entrypoint emits Issue **#69** judgments. Predicates and join rules below remain **normative design** for a future evaluator or for **manual/scripted** pairing. |
| **Operational today (narrator fidelity / stabilization)** | Manual or scripted narrator↔character pairing over audit JSON; **`metadata.narrator_validation_audit_v1`** and related telemetry; substring / parity checks (**`app_turn_rendering`** helpers and operator scripts); **`run_scene_eval_v1`** where character-only predicates apply; headless **`--audit`** + **`structured_eval`** inspection (see Phase **#190** narrator stabilization practice). |

**Specified implementation (deferred — not shipped):** When implemented, **`scene_eval_v2`** would live in **`scene_eval_v2.py`** as **`run_scene_eval_v2`**, **including all Issue #66 v1 judgments** by delegating to **`run_scene_eval_v1`**, then appending v2 predicates (currently **`verbatim_dialogue_contract_v1`**). Until then, treat **`verbatim_dialogue_contract_v1`** as **specified predicate semantics** only — **no** **`run_scene_eval_v2`** judgments are produced by repo tooling.
...
Normative rules for **deterministic cross-row joins** when pairing narrator and character **`*_full.json`** rows — for **manual/scripted offline analysis** today and for **specified** Issue **#69** / **`verbatim_dialogue_contract_v1`** when an evaluator exists:
...
- **Guaranteed (current writers):** Full **`log_character_turn_audit`** / **`log_narrator_render_audit`** rows include this key **only when** the payload contract above holds. In the primary character turn path, that aligns with a **successful** **`process_turn`** and **no** continuity rollback before the full audit write (failed validation / progression rollback / similar paths emit **minimal** snapshots—e.g. **`log_turn_failure_fn`**—**without** this field). **Manual/scripted offline pairing** and **specified** Issue **#69** evaluation use matching values as the **primary** narrator↔character join **when both rows include the key**.
...
**Invocation (when implemented):** **Explicit only.** Headless simulation **does not** and **will not** invoke **`run_scene_eval_v2`** automatically unless a future change wires it; today **no** such function ships. When **`scene_eval_v2`** exists, operators, CI, or scripts would call **`run_scene_eval_v2(session_dir, ...)`** after audits exist.

**Output artifact (stable path — specified for future tooling):** **`_scene_eval_v2.json`** at **`<session_dir>/_scene_eval_v2.json`** (JSON with `scene_eval_version`, `session_dir`, optional `structured_eval_path`, `judgments`, `artifact_path`). Per-turn judgments live **only** in the `judgments` array; **`structured_eval`** rollups (if any) remain **optional summaries** and must not be the sole record. **Absent file is normal** until Issue **#69** tooling ships or an operator drops an ad-hoc artifact.
...
**Operator rule (telemetry vs evaluation layer):** When **`_scene_eval_v2.json`** exists (future shipped evaluator or ad-hoc emission), its **`verbatim_dialogue_contract_v1`** judgments are the **authoritative evaluation-layer record** for that predicate. **Until then**, rely on **manual/scripted pairing** plus **`metadata.narrator_validation_audit_v1`** / Audit v2 dialogue-related telemetry as **diagnostic context** — **not** as a substitute for absent **`scene_eval_v2`** judgments. Dialogue-related **telemetry** (e.g. `prose_dialogue_audit_v1`, Audit v2 dialogue integration) remains **diagnostic** unless explicitly elevated elsewhere. **Disagreement is not a system contradiction** — do not revert to “metric = verdict” (**Issue #60**).
...
### Schema-recoverable parse repair lane (GitHub #250)

**Distinction:** **`parse_retry`** / generic JSON discipline retry = **full regeneration** after **unrecoverable** parse (unreadable JSON, missing beats/motivation, caps, etc.). **Schema-recoverable** ingress failures (readable JSON with meaningful **`beats[]`** + **`motivation`**, but illegal **`semantic_evaluation`** / proposal container fields) route to a **repair lane** first:

1. **Deterministic repair** — strip forbidden proposal helper keys, inject **`character`**, resolve **`semantic_evaluation`** vs root **`semantic_proposals`**; re-validate via existing ingress (**first-pass strict ingress unchanged**).
2. **`parse_schema_repair`** failure stage — when deterministic repair fails and attempts remain; next attempt uses a **repair-biased** prompt (prior emission + field errors), not the generic parse note alone.
3. **Committed success metadata** (`metadata.turn_execution` on character **`*_full.json`**): **`retry_mode`** ∈ {`none`, `deterministic_repair`, `llm_schema_repair`, `regenerate`}, **`first_attempt_rejected`**, **`first_attempt_ingress_error`**, **`first_attempt_semantic_decision`**, **`first_attempt_proposal_kinds`**, **`semantic_decision_changed`**, **`first_attempt_raw_response_digest`**. Pair with **`*_parse_retry_full.json`** / **`*_parse_schema_repair`** failure rows when investigating retry flatten vs ingress loss (**#249**) vs genuine non-emission (**#251**).
...
**Offline tooling (#141)** uses read-only projections from **`character_move_adapters`**: for substring / literal checks over “dialogue + action” text (Issue #29 harness, **`parsed_output_literals_all`** in **`fact_spec.v1`**, narrative **`_narrative.json`** **`character_dialogue` / `character_action`** mirrors), **`format_move_for_legacy_audit_view`** concatenates **speech** then **action** text derived from **`beats[]`** when **`move_schema_version`** is **2**, and falls back to root **`dialogue`** / **`action`** for legacy rows. **`scene_eval_v1`** (**Issue #66**) emits **`scene_eval_version: "2"`** for its output bundle revision (includes **#141** predicate **`parsed_output.move_schema_shape`**); that **`"2"`** is **not** Issue **#69** **`scene_eval_v2`** — see [Offline evaluation layer (Issue #69 — `scene_eval_v2`)](#offline-evaluation-layer-issue-69--scene_eval_v2).
...
- **Character move `semantic_proposals` (GitHub #230 wire + #232 authority):** When present on an ingress-valid v2 move, proposals are **retained on the parsed move** for observability. They express **semantic commit intent** on the wire; **accepted** proposals are the **sole covered-semantic commit source** (#232 validated at `1799415c`). Treat proposal presence alone as **intent evidence**, not committed **`SceneState`** / excursion fact; proof = post-turn **`SceneState`** after successful accept path. Reconstruction-era covered commits (flatten/tag/βʹ) are **removed** ([#235](https://github.com/KizzieFae/Holy_Grail_RP/issues/235)–[#237](https://github.com/KizzieFae/Holy_Grail_RP/issues/237)) — audit must not infer covered roster/excursion commits from prose, tags, or legacy heuristics; use proposal authority + `scene_state_after`.
...
**Rules:** At most one link per `callout_id` in v1. **Default `promote`:** if that `callout_id` is already in `links`, the command **fails** with *duplicate promotion rejected* (unchanged from first ship). **`promote --replace` (reconciliation):** allowed when the **sole** link for an existing `callout_id` must point to a **different** issue—e.g. a validation link to **#125** superseded by a substantive tracked issue. **Explicit flag only;** it **replaces** the one map entry (still one link per callout), updates **`issue_number`**, **`issue_url`**, and sets **`linked_at_utc`** to the time of the write. The link file **does not** track GitHub issue open/close or other lifecycle; **GitHub** remains work authority. **Does not** edit raw per-session `user_callouts_v1.json` (§6). The operator runs **`user_callout_review.py promote`** (or **`promote --replace`** as above) after the appropriate issue exists—**not** from Streamlit. **Atomic** writes (temp + replace) like other audit JSON.
