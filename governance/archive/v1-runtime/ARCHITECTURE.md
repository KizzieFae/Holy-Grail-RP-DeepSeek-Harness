...
**Doctrine anchor:** **[Issue #224](https://github.com/KizzieFae/Holy_Grail_RP/issues/224)** (closed — doctrine record preserved) defines the **stable** continuity-authority reconciliation for exit/off-focal semantics and audit read discipline. **Template cohesion (#245, closed):** production templates require **`cohesion_policy: anchor_only`** only; runtime resolves effective `presence_constraint` via `scene_template_cohesion.py` (anchor **`must_remain`** interim until **[#247](https://github.com/KizzieFae/Holy_Grail_RP/issues/247)** validates anchor relocation). **Living follow-ons:** [#225](https://github.com/KizzieFae/Holy_Grail_RP/issues/225) (narrowed β′ lane), [#226](https://github.com/KizzieFae/Holy_Grail_RP/issues/226) (strict v2 / typed mutation). **Do not** treat unfinished or speculative outcomes from open issues as **implemented** architecture in this file.
...
**Explicitly deferred from this section (see linked issues):** Global **proposal-framework** semantics beyond v1 covered authority, **compatibility-view** governance, **scratch-authority** redesign, and full **ingress/OUTPUT** legality closure — **#226**. Bounded **operational** posture for beat-implied soft exit under remaining **`must_remain`** roles — **#225** (`implemented`, validation pending). **Anchor-led focal relocation** and eventual anchor **`must_remain`** removal — **#247** (post–memory arc). **Semantic proposal arc (#228):** wire (#230 validated), coherence (#231 closed), **covered-semantic authority** (#232 closed), **audit decision schema** (#233 closed at `9ed9144`), **proposal-path certification** (#234 closed at `d127c3c` — deterministic D1–D6 + bounded L2 attempt; live accept not required for `validated`); reconstruction removal **#235** landed; tag→presence decoupling **#236** landed; β′ bridge confirmation **#237** landed; **#238** terminology/audit-doctrine cleanup (narrow hygiene).
...
> **Semantic proposal authority (#232):** Root **`semantic_proposals`** is **governed on the wire** (#230) and **consumed by continuity** on the authoritative path. **Accepted** proposals are the **sole commit source** for v1 covered semantics (`off_focal`, `reentry`, `excursion_lifecycle`). Reconstruction-era covered commit paths (flatten exit/reentry, βʹ) and classifier tag → presence scratch commits are **removed** ([#235](https://github.com/KizzieFae/Holy_Grail_RP/issues/235)–[#237](https://github.com/KizzieFae/Holy_Grail_RP/issues/237); terminology cleanup **#238**). **`scene_exit_detection`** / classifier tags remain **observational** only. **No proposal → no covered commit.** **Reject** → `proposal_legality` retry / forfeit; **no** `process_turn`; **no** heuristic/tag commit fallback.
...
- **Optional (A1 — GitHub #230):** `semantic_proposals` (JSON **array** when present). Wire + emission (#230); **continuity authority** (#232 validated). See **A1 `semantic_proposals`** below.
...
**A1 `semantic_proposals` (GitHub #230 wire + #232 continuity authority)**
...
- **Semantics:** proposals express **semantic commit intent** on the wire; **accepted** proposals are the **sole covered-semantic commit source** (#232). **`beats[]`** carry narrative only. **No proposal → no covered commit.** **Reject** → **`proposal_legality`** retry / forfeit; **no** legacy heuristic/tag commit fallback. Proof of covered commit = post-turn **`SceneState`**, not proposal presence alone.
...
- **Schema-recoverable parse repair (#250):** When JSON loads with meaningful **`beats[]`** + **`motivation`** but strict ingress rejects **`semantic_evaluation`** / proposal shape, the runner attempts **deterministic container repair** on the same attempt (no extra LLM call when repair succeeds). If that fails and attempts remain, **`parse_schema_repair`** logging and a **repair-biased** retry prompt replace generic parse discipline for that class. See **`AUDIT_DOCUMENTATION.md`** (*Schema-recoverable parse repair lane*).

- **Post-parse semantic adjudication (intentionally deferred):** The runtime does **not** run always-on LLM adjudication of RP text (`beats[]`) versus emitted **`semantic_evaluation`** / proposals after parse. Evaluated during the **#249 / #250 / #251** stabilization arc and **omitted by design** — observed evidence did not justify added latency, audit complexity, and LLM cost beyond **deterministic validation**, **#250** schema-recoverable repair, audit visibility, and manual RP pressure testing. **Revisit** only on evidence: recurring harmful silent semantic false negatives, repeated audit misses, or continuity harm from accepted semantic contradictions. See closed **#250** and **#251** deferred notes.

- **Validation-class retries:** For **`[DUPLICATE]`**, **`[BINDING_SLEEPING_SURFACE]`**, **`[INVESTIGATION_ANCHOR]`**, **`[PROPOSAL_SCOPE]` / `[PROPOSAL_INCONSISTENT]` / `[PROPOSAL_COHERENCE]`** (GitHub **#231**), **`[PROPOSAL_LEGALITY]`** (GitHub **#232**), and **progression enforcement** (after **`process_turn`** rollback when the gate is active), the runner may **`continue`** at most **once per failure class** per character slot, and only if the unified cap still allows a later attempt. Further rejection of the same class or other validation failures follow the normal hard-fail path. **Director `decision` and `next_actor` are unchanged** across attempts in the same slot.
...
**Off-focal / reentry signal contract (GitHub #216; #232 covered authority):** **Continuity** (`process_turn`, mutation pipeline, then `_update_scene_state`) remains the **only** routine authority for **committed** focal roster / excursion truth. **Covered semantics** (`off_focal`, `reentry`, `excursion_lifecycle`) commit **only** from **accepted** root **`semantic_proposals`** (#232); reconstruction-era flatten/detect/reentry, tag→presence scratch, and βʹ bridge commits are **removed** ([#235](https://github.com/KizzieFae/Holy_Grail_RP/issues/235)–[#237](https://github.com/KizzieFae/Holy_Grail_RP/issues/237)); **#238** hygiene (terminology/docs) — observational stack retained; **not** merge partners or reject/absence fallbacks. **Non-covered** `process_turn` work (registry, issues, pacing, etc.) remains on the authoritative path. Typed **`excursion_lifecycle`** on moves and **`spatial_transition`** still compose through **`continuity_mutation_pipeline`** for **non-proposal-sourced** mutations; **`session_mutation_candidates`** use the same **D→S→M** validation. **`ConsequenceClassifier`** / **`scene_exit_detection`** / **`turn_metadata` tags** are **observational** relative to covered semantics — **not** covered-semantic commit authority. **Orchestration** reads **mirrored** **`present_characters` / `offstage_characters`**; it does **not** author them. **Perception** (`audience` / audibility on speech beats) governs **verbatim dialogue visibility**, not **`SceneState` presence**. **Narrator `rendered` prose** is **non-authoritative** for continuity (see **`perception_audibility`**, **`AUDIT_DOCUMENTATION.md`** — perception vs continuity-grounded offstage).
