...
- **User callout cross-session review (GitHub #125):** Under the same `rp_audits/` root, **`_user_callout_review_index_v1.json`** (keyed review queue, no `artifact_refs`) and **`_user_callout_issue_links_v1.json`** (keyed `callout_id` → GitHub issue link; sole “promoted” authority). Per-session **`user_callouts_v1.json`** remains evidence (GitHub #55). New callouts from Streamlit still append there and **upsert** the review index. **List / show / dismiss / promote / rebuild / links** are **operator CLI** only: `autogen_rp/python/scripts/user_callout_review.py`. **Default** `promote` rejects a duplicate `callout_id` already in the link map; **`promote --replace`** rewrites the **one** link row when reconciliation requires a different issue target (updates `linked_at_utc`; does not track GitHub lifecycle; raw callouts unchanged). See **`AUDIT_DOCUMENTATION.md`** §6–§8.
