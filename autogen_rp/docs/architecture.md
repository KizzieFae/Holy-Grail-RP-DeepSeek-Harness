...
- **Progression enforcement** (`progression_enforcement.py`) may require a structural delta (Q1–Q4) after continuity `process_turn`; it **does not** classify moves. Empty or thin **`consequences`** on continuity turn metadata are corrected in **`continuity_consequence_classifier.py`** (deterministic rules, continuity still authoritative)—**not** by weakening the gate or editing Q1–Q4. See `python/rp_app/ARCHITECTURE.md` (*Progression enforcement vs continuity classification*).
