...
- **Phase A (writes):** `facade`, `writes`, `storage` — all commit-time episodic writes go through the facade; **`storage`** is the only path that calls `CharacterStateManager.remember_event`. **Observer** episodic lines are written only for names in **`event_knowledge_recipients`** after **`normalize_move_audibility`** (`perception_audibility.py`). **`present_characters`** for that pass: `continuity_manager.scene_state.present_characters` when non-empty, else the turn’s cast / `char_names`. **Actor episodic interpretation** (GitHub **#208**) uses a deterministic ladder: prefer verbatim **`director_model_reason`** when present and non-empty, else merged **`decision["reason"]`**, else move **`motivation`** `goal`/`tactic` text; normative detail in **`python/rp_app/ARCHITECTURE.md`**.
