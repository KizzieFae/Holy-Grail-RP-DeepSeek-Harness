...
Writes JSON artifacts under autogen_rp/tools/ only.
"""
...
RP_AUDITS = REPO / "autogen_rp" / "python" / "rp_app" / "data" / "rp_audits"
TOOLS = REPO / "autogen_rp" / "tools"
...
def build_registry(rows: list[dict], refs: set[int]) -> dict:
    ...
    return {
        ...
        "repo_relative_root": "autogen_rp/python/rp_app/data/rp_audits",
        "scenarios": out_rows,
        ...
        candidates.append(
            {
                "session_id": d.name,
                "session_number": n,
                "repo_relative_path": f"autogen_rp/python/rp_app/data/rp_audits/{d.name}",
        ...
        json.dumps(
            {
                ...
                "generated_at_utc": datetime.now(timezone.utc).isoformat(),
                "registry_snapshot_file": str(snap_path.relative_to(REPO)).replace("\\", "/"),
