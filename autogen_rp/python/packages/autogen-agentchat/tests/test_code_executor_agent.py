...
""".strip()
    ...
    def _norm_output(text: str) -> str:
        # Windows subprocess tracebacks use CRLF; normalize for assertions.
        return text.replace("\r\n", "\n")
    correct_code_block = """
numbers = [10, 20, 30, 40, 50]
mean = sum(numbers) / len(numbers)
...
```""",
    ...
    async for message in agent.on_messages_stream(messages, CancellationToken()):
        if isinstance(message, CodeGenerationEvent) and message_id == 0:
            ...
            assert (
                incorrect_code_result in _norm_output(message.to_text().strip())
            ...
            assert (
                _norm_output(message.to_text().strip()) == correct_code_result
