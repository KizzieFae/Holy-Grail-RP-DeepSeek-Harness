...
def test_build_character_turn_prompt_includes_expected_sections_and_rules() -> None:
    ...
    assert "Root ``semantic_evaluation`` (required every beat)" in prompt
    assert "Do not emit root ``semantic_proposals``" in prompt
    assert "prose implication is insufficient" not in prompt
    assert "``covered_change``" in prompt and "``no_covered_change``" in prompt
    ...
    assert '"move_schema_version": 2' in prompt
    assert 'Positive example: {"move_schema_version": 2, "beats":' in prompt
    assert 'Negative example: {"move_schema_version": 2, "beats":' in prompt
