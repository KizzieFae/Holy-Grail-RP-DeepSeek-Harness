#!/usr/bin/env python3
"""Extract Willow departure experiment matrix from audited session artifacts (#227)."""
...
from willow_departure_experiment_extract import (  # noqa: E402
    extract_session,
    summarize_willow_rows,
)


def _default_manifest() -> Path:
    return _PY_ROOT / "data" / "issue227" / "i227_willow_departure_probe_manifest_v1.json"
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Extract Willow departure experiment matrix from audited Willow turns."
    )
    p.add_argument("--session-dir", type=Path, help="Path to rp_audits/session_NNN directory")
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="Probe manifest JSON (default: data/issue227/i227_willow_departure_probe_manifest_v1.json)",
    )
    p.add_argument(
        "--out-jsonl",
        type=Path,
    ...
    p.add_argument("--summary-out", type=Path, default=None, help="Optional JSON summary path")
    args = p.parse_args()
    ...
    summary = summarize_willow_rows(rows)
