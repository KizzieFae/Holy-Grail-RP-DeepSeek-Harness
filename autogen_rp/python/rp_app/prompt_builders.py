...
- Covered semantic intent (``semantic_proposals``): MUST emit root ``semantic_proposals`` when this turn has off-focal, reentry, or excursion lifecycle intent. Omit ONLY when no covered semantic intent exists. Narrative beats alone do not change continuity focal presence or excursion state; prose implication is insufficient for off-focal, reentry, or excursion lifecycle semantics.
- Root ``semantic_proposals`` (array): semantic commit intent only — not a commit; continuity evaluates and accepts or rejects this turn. Each item: ``kind`` (``off_focal`` | ``reentry`` | ``excursion_lifecycle``), ``character`` (required); ``operation`` (``open`` | ``update`` | ``close``) required only for ``excursion_lifecycle``. Proposals declare covered semantic commit intent for continuity to evaluate (emission alone is not proof of commit). Do not emit root ``presence_changes``, ``excursion_lifecycle``, or ``spatial_transition`` (ingress rejects them).
