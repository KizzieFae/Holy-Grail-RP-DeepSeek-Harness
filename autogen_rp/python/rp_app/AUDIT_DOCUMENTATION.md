...
**Character prompt proposal-schema teaching (#249, canonical default `v1_next7`):** Production character system prompts teach **`semantic_evaluation.proposals`** shape via `prompt_topology_issue240.py` (marker `proposal_schema_teaching_v249_a`). This aligns model output with **`issue240_semantic_evaluation.validate_issue240_semantic_evaluation_ingress`** — observational distinction: **schema reject** (forbidden keys / missing `character`) vs **semantic reject** vs **authority reject** remains in audit/validation metadata, not in #243 eval. Ontology/retry/missed-emission investigations: **#251** (canonical severance doctrine in `v1_next7_issue251_awareness_clean` — see `validation_runs/issue251/CANONICAL_DOCTRINE.md`; adjudicated replay via `i251_replay_adjudication.py`), **#250**. Evidence: `validation_runs/issue249/`, `validation_runs/issue251/`.
