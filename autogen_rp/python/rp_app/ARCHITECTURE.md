...
> **Semantic proposal authority (#232):** Root **`semantic_proposals`** is **governed on the wire** (#230) and **consumed by continuity** on the authoritative path. **Accepted** proposals are the **sole commit source** for v1 covered semantics (`off_focal`, `reentry`, `excursion_lifecycle`). Reconstruction-era covered commit paths (flatten exit/reentry, βʹ) are **removed** ([#235](https://github.com/KizzieFae/Holy_Grail_RP/issues/235)); classifier tag → presence decoupling remains ([#236](https://github.com/KizzieFae/Holy_Grail_RP/issues/236)–[#238](https://github.com/KizzieFae/Holy_Grail_RP/issues/238)). **No proposal → no covered commit.** **Reject** → `proposal_legality` retry / forfeit; **no** `process_turn`; **no** reconstruction fallback.
...
**Off-focal / reentry signal contract (GitHub #216; #232 covered authority):** **Continuity** (`process_turn`, mutation pipeline, then `_update_scene_state`) remains the **only** routine authority for **committed** focal roster / excursion truth. **Covered semantics** (`off_focal`, `reentry`, `excursion_lifecycle`) commit **only** from **accepted** root **`semantic_proposals`** (#232); reconstruction-era flatten/detect/reentry commit paths are **removed** ([#235](https://github.com/KizzieFae/Holy_Grail_RP/issues/235)); classifier tag → presence and βʹ remain for ([#236](https://github.com/KizzieFae/Holy_Grail_RP/issues/236)–[#238](https://github.com/KizzieFae/Holy_Grail_RP/issues/238)) — **not** merge partners or reject/absence fallbacks. **Non-covered** `process_turn` work (registry, issues, pacing, etc.) remains on the authoritative path. Typed **`excursion_lifecycle`** on moves and **`spatial_transition`** still compose through **`continuity_mutation_pipeline`** for **non-proposal-sourced** mutations; **`session_mutation_candidates`** use the same **D→S→M** validation. **`ConsequenceClassifier`** / **`scene_exit_detection`** / tags feeding legacy presence scratch are **heuristic observation** relative to covered semantics — **not** covered-semantic commit authority post-#232. **Orchestration** reads **mirrored** **`present_characters` / `offstage_characters`**; it does **not** author them. **Perception** (`audience` / audibility on speech beats) governs **verbatim dialogue visibility**, not **`SceneState` presence**. **Narrator `rendered` prose** is **non-authoritative** for continuity (see **`perception_audibility`**, **`AUDIT_DOCUMENTATION.md`** — perception vs continuity-grounded offstage).
