...
> **Semantic proposal authority (#232):** Root **`semantic_proposals`** is **governed on the wire** (#230) and **consumed by continuity** on the authoritative path. **Accepted** proposals are the **sole commit source** for v1 covered semantics (`off_focal`, `reentry`, `excursion_lifecycle`). Reconstruction-era covered commit paths (flatten exit/reentry, βʹ) and classifier tag → presence scratch commits are **removed** ([#235](https://github.com/KizzieFae/Holy_Grail_RP/issues/235), [#236](https://github.com/KizzieFae/Holy_Grail_RP/issues/236), [#237](https://github.com/KizzieFae/Holy_Grail_RP/issues/237) confirmation). **`scene_exit_detection`** / classifier tags remain **observational** only. **#238** covers infrastructure audit. **No proposal → no covered commit.** **Reject** → `proposal_legality` retry / forfeit; **no** `process_turn`; **no** reconstruction or tag fallback.
