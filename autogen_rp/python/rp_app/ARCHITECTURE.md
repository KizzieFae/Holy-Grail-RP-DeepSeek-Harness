...
- **Mutation authority (runtime turns):** For normal character turns, **`ContinuityManager.process_turn`** runs **`compose_resolved_mutations` → `validate_resolved_mutations_globally` → `apply_resolved_mutations`** — that pipeline is the **authoritative mutation path for runtime turns**. The same codebase also exposes **`open_excursion` / `update_excursion` / `close_excursion`**, direct assignment to **`SceneState.location`** (e.g. scene bootstrap / headless init), and the module function **`apply_excursion_close_reintegration_mutation`**. Those entry points **bypass** pipeline validation; they are **not** the runtime-turn authority surface and are **unsafe** for parity with production turn rules unless callers replicate the same checks.
