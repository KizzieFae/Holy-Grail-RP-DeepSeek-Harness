...
- **Operational retrieval pilot (closed):** **Accepted** product baseline for authored retrieval is **minimal character `lore_facts`** + **template `role_slots` + refined `premise`** in the **operational manifest** (`autogen_rp/python/data/retrieval/manifests/operational_pilot.json` → `compiled/operational_pilot_v3.json`), wired only via **`RP_RETRIEVED_CONTEXT_INDEX`**. **Headless** now passes **`scene_template_id`** (`prepare_headless_session`, scenario JSON, `run_scene_simulation_llm.py --scene-template-id`) so template rows select the same way as Streamlit. A **low-tension situational cap** on `scene_template` rows was **evaluated and not adopted** (reverted); selector remains **fixed per-`source_kind` subcaps** only. Runbook: `autogen_rp/python/data/retrieval/OPERATIONAL_RETRIEVAL_PILOT.md`; closeout notes: `autogen_rp/python/RP_SETUP_TODO.md` (Operational retrieval pilot section).
